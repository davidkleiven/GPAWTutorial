...
def main(prefix, start, startfile, initfunc, dx=30.0, steps=0, update_freq=0, dt=0.01):
    ...
    if startfile is not None:
        ...
        elif initfunc == 'precipitate_flat':
            chgl.from_npy_array(precipitate_flat(L))
...
def precipitate_flat(L):
    conc = np.zeros((L, L, L))
    start = 20
    end = 40
    start_y = 25
    end_y = 35
    conc[start:end, start_y:end_y, start:end] = 1.0

    eta1 = np.zeros((L, L, L))
    eta1[start:end, start_y:end_y, start:end] = 0.8
    eta2 = np.zeros((L, L, L))
    eta3 = np.zeros((L, L, L))
    return [conc, eta1, eta2, eta3]
...
if __name__ == "__main__":
    ...
    if startfile is not None:
        ...
        assert initfunc in ['matsuda', 'precipitate_square', 'prec_square_bck', 'precipitate_flat']

    main(prefix, start, startfile, initfunc, dx, num_steps, update_freq, dt)
